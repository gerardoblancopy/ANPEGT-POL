"""Opposition research corpus source."""
...
class OppositionSource:
    """Reads opposition research documents from .txt files."""

    source_type: str = "opposition"

    def fetch(self, config: dict) -> list[CorpusDocument]:
        """Fetch opposition research documents from text files.
        ...
        if not path.is_dir():
            logger.warning(
                "OppositionSource: path %s is not a directory", path
            )
            return []

        source_id_prefix = config.get("id", "opposition")
        ...
        for filepath in sorted(path.glob("*.txt")):
            try:
                ...
                logger.warning(
                    "OppositionSource: error reading %s: %s", filepath, exc
                )

        logger.info(
            "OppositionSource: fetched %d documents from %s",
            len(documents),
            path,
        )
        return documents
